...
bool Dijkstra::Run(GridBasedGraph& graph, int startX, int startY, int endX, int endY, IsBlocked isBlocked, GetCost getcost)
{
	//reset enerything
		...
		else
		{
			for (auto neighbor : N->neighbors)
			{
				...
				if (!neighbor->opened)
				{
					...
					// neighbor->g = N->g + getcost(node->column, node->row, neighbor->column, neighbor->row);
